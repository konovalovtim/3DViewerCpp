...
MainWindow::MainWindow(QWidget *parent)
  ...
  settings = new QSettings("2Danger_Masters", "3DViewer_v2.0", this);
  load_Settings();
}
...
void MainWindow::on_button_open_path_clicked() {
  ...
  std::string path = __buffpath.toStdString();
...
//  if (!parsing_object_t(&ui->widget->model, path)) {
  s21::Model model2;
  model2.clear();
  model2.OpenObject(path);
  model2.NormalizationAndCentralize();
...
void MainWindow::on_rotate_x_returnPressed() {
  ...
  ui->widget->model.Rotate(val, s21::Model::xAxis);
